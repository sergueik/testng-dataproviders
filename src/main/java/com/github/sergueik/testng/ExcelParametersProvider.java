...
public class ExcelParametersProvider {
	...
	public static Object[][] createData_from_OpenOfficeSpreadsheet(final ITestContext context, final Method method) {
		if (debug) {
			utils.getCallerInfo(context, method);
		}
		...
		if (parameters != null) {
			...
			if (!filePath.matches("^[/\\\\].*") && !filePath.matches("^(?i:[A-Z]):.*")) {
				filePath = String.format("%s/%s", System.getProperty("user.dir"), filePath);
			}
	...
	public static Object[][] createDataFromExcel2007(final ITestContext context, final Method method) {
		if (debug) {
			utils.getCallerInfo(context, method);
		}
		DataFileParameters parameters = method.getAnnotation(DataFileParameters.class);
		...
		if (debug) {
			System.err.println("Data Provider Caller Suite: " + context.getCurrentXmlTest().getSuite().getName());
			System.err.println("Data Provider Caller Test: " + context.getCurrentXmlTest().getName());
		...
		/*
		 * @SuppressWarnings("deprecation") Map<String, String> parameters =
		 * (((TestRunner) context).getTest()) .getParameters(); for (String key :
		 * parameters.keySet()) { System.out.println("Data Provider Caller Parameter: "
	...
	public static Object[][] createDataFromExcel2003(final ITestContext context, final Method method) {
	...
	private static String amendFilePath(String filePath) {
		if (debug) {
			System.err.print(String.format("Amending the %s with %s", filePath, testEnvironment));
		}
		// Inject the directory into the file path
		String updatedFilePath = filePath.replaceAll("^(.*)/([^/]+)$", String.format("$1/%s/$2", testEnvironment));
		if (debug) {
